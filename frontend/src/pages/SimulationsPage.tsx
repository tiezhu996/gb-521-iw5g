...
export function SimulationsPage() {
  ...
  const launch = async (scenario?: number) => {
    const targetScenario = scenario ?? scenarioId;
    if (!targetScenario) return;
    setStarting(true);
    try { const run = await start(targetScenario); message.success(`推演 #${run.id} 已完成并保存结果快照`); } catch (error) { reportError(error, '推演未能启动'); } finally { setStarting(false); }
  };
  const columns: ColumnsType<SimulationRun> = [
    ...
    { title: '状态', dataIndex: 'run_status', width: 210, render: (value, row) => <span className="status-cell"><StatusBadge status={value} />{!row.network_fresh && <StatusBadge status="stale" />}</span> },
  ...
  return (
    <div className="page">
      ...
      {selected && <section className="workspace-section result-detail" aria-labelledby="result-heading"><div className="section-heading"><div><span className="section-index">03</span><h2 id="result-heading">运行 #{selected.id} 计算证据</h2></div><span className="status-cell"><StatusBadge status={selected.run_status} />{!selected.network_fresh && <StatusBadge status="stale" />}</span></div>{!selected.network_fresh && <NetworkFreshnessBanner changes={selected.network_changes ?? []} action={<p className="stale-action-hint">请保持方案「{selected.scenario?.name ?? `#${selected.scenario_id}`}」不变，<Button size="small" type="primary" icon={<Play size={14} />} loading={starting} disabled={!hasRole('engineer', 'admin')} onClick={() => void launch(selected.scenario_id)}>用同一方案重新发起推演</Button> 后以新一轮结果为准。</p>} />}{selected.risk_confirmed_at && <Alert className="section-alert" type="success" showIcon message={`风险证据已于 ${formatDateTime(selected.risk_confirmed_at)} 经人工确认（确认记录原样保留）`} />}<div className="result-metrics"><div><span>迭代轮次</span><strong>{selected.iteration_count}</strong></div><div><span>最终残差</span><strong>{formatNumber(selected.residual, 6)}</strong></div><div><span>风险证据</span><strong>{selected.risk_flags_json?.length ?? 0}</strong></div><div><span>算法版本</span><strong>{selected.algorithm_version}</strong></div></div><ResidualChart values={selected.residuals_json ?? []} /><h3>联锁规则证据</h3><RiskEvidenceTable risks={selected.risk_flags_json ?? []} /></section>}
    </div>
  );
}
